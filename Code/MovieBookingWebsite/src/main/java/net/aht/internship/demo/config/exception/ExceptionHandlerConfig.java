...
import net.aht.internship.demo.adapter.web.base.RestData;
...
public class ExceptionHandlerConfig {
    ...
    protected ResponseEntity<RestData<?>> handleInvalidException(InvalidException ex) {
        String resolvedReason = this.messageSource.getMessage(ex.getUserMessage(), null,
    ...
    protected ResponseEntity<RestData<?>> handConstraintException(ConstraintViolationException ex) {
        String message;
        try {
            // LocalContextHolder.getLocale() : get java object present ,use define language
            // and country that application is using
    ...
    /*
     * @ExceptionHandler(value = {BindException.class}) protected
     * ResponseEntity<RestData<?>> handBindingException(BindException ex) { String
     * message = ""; if (ex.getFieldError() != null) { try { message =
     * messageSource.getMessage(Objects.requireNonNull(ex.getFieldError().
     * getDefaultMessage()), null, LocaleContextHolder.getLocale()); } catch
     * (Exception exception) { message =
     * messageSource.getMessage("An error occurred", null,
     * LocaleContextHolder.getLocale()); } } return
     * VsResponseUtil.error(HttpStatus.BAD_REQUEST, message, ex.getCause() != null ?
    ...
    protected ResponseEntity<RestData<?>> handlerRefreshTokenException(HttpClientErrorException.Forbidden ex) {
        ...
        String message;
        try {
            message = messageSource.getMessage(Objects.requireNonNull(ex.getMessage()), null,
                    LocaleContextHolder.getLocale());
        } catch (Exception exception) {
            message = exception.getMessage();
        }
